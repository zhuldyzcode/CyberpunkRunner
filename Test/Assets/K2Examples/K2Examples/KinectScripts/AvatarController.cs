...
{	
    ...
    [Tooltip("Whether to keep the last position and rotation, when the user gets lost, or not.")]
...
//            jointPosition.y - offsetPos.y + kinectManager.sensorHeight,  //!! this should be better investigated .. 
    ...
    {	
        ...
        {
            UserID = userId;
            //Debug.Log("Updating avatar " + playerIndex + " user Id to: " + userId);
...
//		}
    ...
    public virtual void ResetInitialTransform(Vector3 position, Quaternion rotation)
    {
        ...
        if (offsetNode != null)
        {
            //bodyRootOffsetPos = bodyRootPosition - offsetNode.transform.position;
	...
	protected void MoveAvatar(Int64 UserID)
	{
		...
		if(posRelativeToCamera)
		{
            ...
            else
            {
                ...
                Vector3 bodyRootPos = bodyRoot != null ? bodyRoot.position : transform.position;
                Vector3 userLocalPos = kinectManager.GetUserKinectPosition(UserID);
        ...
        // removed fixed bone indices - thanks to Martin Cvengros!
        ...
        var biPelvis = GetBoneIndexByJoint(KinectInterop.JointType.SpineBase, false);  // you may replace 'false' with 'mirroredMovement'
...
//					}
	...
	protected Quaternion Kinect2AvatarRot(Quaternion jointRotation, int boneIndex)
	{
        ...
        if (offsetNode != null)
		{
            ...
            matAvatar.SetTRS(transform.position - offsetNode.transform.position, newRotation, Vector3.one);
            Matrix4x4 matOffsetNode = Matrix4x4.identity;
            matOffsetNode.SetTRS(offsetNode.transform.position, offsetNode.transform.rotation, Vector3.one);

            Matrix4x4 matOffset = matOffsetNode * matAvatar;
            newRotation = matOffset.rotation;
        }

        //Debug.Log($"Avatar {playerIndex} rot: {newRotation.eulerAngles:F0}, offsetNode: {offsetNode}");
	...
	protected Vector3 Kinect2AvatarPos(Vector3 jointPosition, bool bMoveVertically, bool bMoveHorizontally)
	{
		...
		if(offsetNode != null)
		{
            ...
            Matrix4x4 matAvatar = Matrix4x4.identity;
            matAvatar.SetTRS(bodyRootOffsetPos + newPosition, Quaternion.Inverse(offsetNode.transform.rotation) * transform.rotation, Vector3.one);
            Matrix4x4 matOffsetNode = Matrix4x4.identity;
            matOffsetNode.SetTRS(offsetNode.transform.position, offsetNode.transform.rotation, Vector3.one);
        ...
        //Debug.Log($"Avatar {playerIndex} pos: {newPosition:F2}, offsetNode: {offsetNode}");
