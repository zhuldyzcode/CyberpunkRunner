...
//			return string.Empty;
    ...
    // converts current body frame to a single k4b-csv line. returns empty string if there is no body data
    public static string GetBodyFrameAsK4b(SensorData sensorData, ref BodyFrameData bodyFrame, ref long liRelTime, Vector3 spaceScale, ref float fUnityTime, char delimiter)
    {
        // check for invalid sensor data and if the frame is still the same
        if (sensorData == null)
            return string.Empty;
        ...
        int jointCount = JointTypeK4b.Length;
        sbBuf.Append(sensorData.bodyCount).Append(delimiter);
        ...
        for (int i = 0; i < sensorData.bodyCount; i++)
        {
            ...
            if (bodyFrame.bodyData[i].bIsTracked != 0)
            {
                ...
                for (int j4b = 0; j4b < jointCount; j4b++)
                {
                    int j = (int)JointTypeK4b[j4b];

                    if(j == -1)
                    {
                        sbBuf.Append((int)TrackingState.NotTracked).Append(delimiter);
                        continue;
                    }
    ...
    // sets current body frame from the given k4b-csv line. returns the number of bodies
    public static bool SetBodyFrameFromK4b(string sCsvLine, string sDelim, SensorData sensorData, ref BodyFrameData bodyFrame,
        ref Matrix4x4 sensorToWorld, bool bIgnoreZCoords)
    {
        long liRelTime = 0;
        ...
        int.TryParse(alCsvParts[4], out iSX);
        int.TryParse(alCsvParts[5], out iSY);
        int.TryParse(alCsvParts[6], out iSZ);
        ...
        for (int i = 0; i < bodyCount; i++)
        {
            ...
            if (bIsTracked != 0)
            {
                if (jointCount != JointTypeK4b.Length)
                {
                    // set the other joints as not tracked
                    for (int j4b = jointCount; j4b < JointTypeK4b.Length; j4b++)
                    {
                        int j = (int)JointTypeK4b[j4b];
                        if(j >= 0)
                            bodyFrame.bodyData[i].joint[j].trackingState = TrackingState.NotTracked;
                ...
                for (int j4b = 0; j4b < jointCount; j4b++)
                {
                    ...
                    {
                        int.TryParse(alCsvParts[iIndex], out iTrackingState);
                        iIndex++;     // skip tracking state

                        if (iTrackingState != (int)TrackingState.NotTracked && alCsvParts.Length >= (iIndex + 6))
                        {
                            iIndex += 3;  // skip kinect pos
                            iIndex += 3;  // skip normal rot
                        }
                    ...
                    if (alCsvParts.Length >= (iIndex + 1))
                    {
                        ...
                        float jPosZ = (bIgnoreZCoords && j > 0) ? bodyFrame.bodyData[i].joint[0].kinectPos.z : jointData.kinectPos.z;
                        ...
                        if (j == 0)
                        {
                            ...
                            bodyFrame.bodyData[i].normalRotation = jointData.normalRotation;
                            bodyFrame.bodyData[i].mirroredRotation = jointData.mirroredRotation;
        ...
        return true;  // (uint)bodyCount;
    }

    // k4b joint types, with azure k4a joints added
    private static readonly JointType[] JointTypeK4b = new JointType[]
    {
        JointType.SpineBase,      // Pelvis
        (JointType)(-1),          // SpineNaval
        JointType.SpineMid,       // SpineChest
        JointType.Neck,           // Neck
        JointType.Head,           // Head

        JointType.SpineShoulder,  // ClavicleLeft
        JointType.ShoulderLeft,   // ShoulderLeft
        JointType.ElbowLeft,      // ElbowLeft
        JointType.WristLeft,      // WristLeft
        JointType.HandLeft,       // HandLeft

        JointType.SpineShoulder,  // ClavicleRight
        ...
        (JointType)(-1),          // EyeLeft
        (JointType)(-1),          // EarLeft
        (JointType)(-1),          // EyeRight
        (JointType)(-1),          // EarRight
...
//		return true;
    ...
    public static void TransformTexture(Texture srcTex, RenderTexture tgtTex, int rotationAngle = 0, bool flipX = false, bool flipY = false, bool keepST = false)
    {
        ...
        if (isPortraitMode)
        {
            ...
            texST = !keepST ? GetTexST(tgtAspect, camAspect) : new Vector4(1f, 1f, 0f, 0f);
        }
        else
        {
            transMat = GetTransformMat(rotationAngle, flipX, flipY);
            texST = !keepST ? GetTexST(camAspect, tgtAspect) : new Vector4(1f, 1f, 0f, 0f);
        }
