...
//	};
	...
	{
		public byte id;
		[MarshalAs(UnmanagedType.ByValArray, SizeConst = OBT_MAX_JOINTS)]
...
//		return 1;
	...
	public KinectInterop.SensorData OpenDefaultSensor (KinectInterop.FrameSource dwFlags, float sensorAngle, bool bUseMultiSource)
	{
		...
		if((dwFlags & KinectInterop.FrameSource.TypeColor) != 0)
		{
            ...
            while ((hr != 0 || GetColorWidth() == 0 || GetColorHeight() == 0) &&
                Time.realtimeSinceStartup <= waitTillTime)
            {
                if (hr == 0)
                    ReleaseColorFrame();
		...
		if((dwFlags & KinectInterop.FrameSource.TypeDepth) != 0)
		{
            ...
            while ((hr != 0 || GetDepthWidth() == 0 || GetDepthHeight() == 0) &&
                Time.realtimeSinceStartup <= waitTillTime)
            {
                if (hr == 0)
                    ReleaseDepthFrame();
...
//		}
		...
		{
			waitTimeBeforeRemove = KinectManager.Instance.GetWaitTimeBeforeRemove();
		}
...
//				int bodySize2 = GetBodyDataSize();
				...
				for(int i = 0; i < sensorData.bodyCount; i++)
				{
					...
					if (!bodyIdToIndex.ContainsKey(uBodyId) && eIndex >= 0) 
					{
					...
					if(obtBody.status != (byte)ObtBodyStatus.BODY_STATUS_NOT_TRACKING)
					{
						...
						for(int j = 0; j < sensorData.jointCount; j++)
						{
							...
							if(j == 0)
							{
                                bodyFrame.bodyData[bi].kinectPos = jointData.kinectPos;
