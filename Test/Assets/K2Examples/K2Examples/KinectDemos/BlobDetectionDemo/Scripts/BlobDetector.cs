...
public class BlobDetector : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (lastDepthFrameTime != kinectManager.GetDepthFrameTime())
        {
            ...
            if ((Time.time - lastCheckTime) >= timeBetweenChecks)
            {
                ...
                if (savedFrames != null)
                {
                    ushort[] depthFrame = kinectManager.GetRawDepthMap();

                    if(depthFrame != null)
                    {
                        KinectInterop.CopyBytes(depthFrame, sizeof(ushort), savedFrames[frameIndex], sizeof(ushort));
    ...
    private void DetectBlobsInRawDepth()
    {
        ...
        ushort[] rawDepth = kinectManager.GetRawDepthMap();
        if (rawDepth == null)
            return;
